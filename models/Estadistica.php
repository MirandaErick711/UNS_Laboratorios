...
class Estadistica{
    ...
    // Obtiene indicadores de los laboratorios
    public function obtenerIndicadores(): array {
        $sql = "
            SELECT
                COUNT(*) AS total_laboratorios,
                SUM(CASE WHEN estado = 'Operativo' THEN 1 ELSE 0 END) AS laboratorios_operativos,
                SUM(CASE WHEN estado = 'Mantenimiento' THEN 1 ELSE 0 END) AS laboratorios_mantenimiento,
                SUM(CASE WHEN estado = 'Inactivo' THEN 1 ELSE 0 END) AS laboratorios_inactivos
            FROM laboratorios
        ";
    ...
    // Obtiene las reservas aprobadas del mes
    ...
    // Obtiene las reservas rechazadas del mes
    ...
    // Obtiene el uso de cada laboratorio
    public function obtenerUsoPorLaboratorio(): array {
        $sql = "
            SELECT
                ...
                COUNT(
                    CASE
                        WHEN r.estado = 'Aprobada' THEN r.id_reserva
                    END
                ) AS reservas_mes
            FROM laboratorios l
            ...
            GROUP BY l.id_laboratorio, l.nombre, l.estado
