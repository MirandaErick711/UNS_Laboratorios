...
<nav class="navbar navbar-dark" style="background-color: var(--uns-rojo);">
        ...
        <div class="modal-content">
            ...
            <form id="formReserva">
                <div class="modal-body">
                    <div id="reservaAlert" class="alert py-2 d-none" role="alert"></div>
...
<div class="modal fade" id="modalAvisos" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div id="cargandoAvisos" class="text-center py-4">
                    <div
                        class="spinner-border"
                        style="color: var(--uns-rojo);"
                        role="status"
                    ></div>
                    <p class="text-muted mt-2 mb-0">Cargando avisos...</p>
                </div>

                <div id="sinAvisos" class="text-center text-muted py-4 d-none">
                    <i class="bi bi-bell-slash fs-2"></i>
                    <p class="mt-2 mb-0">No tienes avisos.</p>
                </div>

                <div id="listaAvisos" class="list-group d-none"></div>
            </div>
        </div>
    </div>
</div>
...
async function cargarLaboratorios() {
    ...
    try {
        ...
        resultado.data.forEach(function(laboratorio) {
            ...
            option.textContent = laboratorio.nombre + ' - Capacidad: ' + laboratorio.capacidad;
...
async function cargarAvisos() {
    const cargando = document.getElementById('cargandoAvisos');
    const sinAvisos = document.getElementById('sinAvisos');
    const lista = document.getElementById('listaAvisos');

    try {
        const respuesta = await fetch('../api/avisos.php');
        const resultado = await respuesta.json();

        if (!resultado.success) {
            throw new Error(resultado.message || 'Error al cargar avisos');
        }
        ...
        resultado.data.forEach(function(aviso) {
            ...
            const estadoLeido = Number(aviso.leido) === 0 ? 'fw-bold' : '';
        ...
        sinAvisos.innerHTML = `
            ...
            <p class="mt-2 mb-0">No se pudieron cargar los avisos.</p>
        `;
    }
}
...
async function cargarContadorAvisos() {
    try {
        ...
        mostrarContadorAvisos(Number(resultado.no_leidos));
    } catch (error) {
